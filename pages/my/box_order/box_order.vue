	...
	export default {
		...
		methods: {
			...
			get_order() {
				...
				//获取全部
				this.api.box_order(30, ++this.page).then(res => {
			...
			get_unpaid_order() {
				...
				this.api.box_order(30, ++this.page, 2).then(res => {
					res.data.data.forEach(ele => {
						if(ele.box==null){
							this.all.push({
								create_time: ele.create_at,
			...
			get_ok_order() {
				...
				this.api.box_order(30, ++this.page, 1).then(res => {
					res.data.data.forEach(ele => {
						if(ele.box==null){
							this.all.push({
								create_time: ele.create_at,
			...
			get_cancel_order() {
				...
				this.api.box_order(30, ++this.page, 4).then(res => {
					res.data.data.forEach(ele => {
					if(ele.box==null){
						this.all.push({
							create_time: ele.create_at,
